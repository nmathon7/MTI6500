...
#           sinon c'est par défaut en mode sabotages.
...
declare -a SERVEURS_TOUS=('000.000.000.131' '000.000.000.132' '000.000.000.133' '000.000.000.134' '000.000.000.135' '000.000.000.000');
declare -a SERVEURS=();
declare -a LISTE=();
...
do
      ...
      then
         ...
         if [ "$MODE" = "-o" ] 
            then   
               ...
               $HIVE_HOME/bin/hive -f $HOME/scripts/crud2.hql --hivevar collectl_dtdebut=$COLLECTL_DTDEBUT --hivevar collectl_hrdebut=$COLLECTL_HRDEBUT --hivevar repexport=$REP_EXPORT --hivevar scpfichier=$SCP_FICHIER --hivevar serv1=${LISTE[0]} --hivevar serv1var1=$serv1var1 --hivevar serv1var2=$serv1var2 --hivevar serv2=${LISTE[1]} --hivevar serv2var1=$serv2var1 --hivevar serv2var2=$serv2var2 --hivevar serv3=${LISTE[2]} --hivevar serv3var1=$serv3var1 --hivevar serv4=${LISTE[3]} --hivevar serv5=${LISTE[4]} --hivevar serv5var1=$serv5var1 --hivevar serv5var2=$serv5var2 --hivevar serv6=${LISTE[5]} --hivevar serv6var1=$serv6var1 --hivevar serv6var2=$serv6var2 --hivevar serv7=${LISTE[6]} --hivevar serv8=${LISTE[7]} --hivevar serv8var1=$serv8var1 --hivevar serv9=${LISTE[8]} --hivevar serv9var1=$serv9var1 --hivevar serv10=${LISTE[9]}
			   
         else

               echo "MODE SABOTAGES"
               $HIVE_HOME/bin/hive -f $HOME/scripts/prod.hql --hivevar collectl_dtdebut=$COLLECTL_DTDEBUT --hivevar collectl_hrdebut=$COLLECTL_HRDEBUT --hivevar repexport=$REP_EXPORT --hivevar scpfichier=$SCP_FICHIER --hivevar serv1=${LISTE[0]} --hivevar serv1var1=$serv1var1 --hivevar serv1var2=$serv1var2 --hivevar serv2=${LISTE[1]} --hivevar serv2var1=$serv2var1 --hivevar serv2var2=$serv2var2 --hivevar serv3=${LISTE[2]} --hivevar serv3var1=$serv3var1 --hivevar serv4=${LISTE[3]} --hivevar serv5=${LISTE[4]} --hivevar serv5var1=$serv5var1 --hivevar serv5var2=$serv5var2 --hivevar serv6=${LISTE[5]} --hivevar serv6var1=$serv6var1 --hivevar serv6var2=$serv6var2 --hivevar serv7=${LISTE[6]} --hivevar serv8=${LISTE[7]} --hivevar serv8var1=$serv8var1 --hivevar serv9=${LISTE[8]} --hivevar serv9var1=$serv9var1 --hivevar serv10=${LISTE[9]}
   ...
   pdsh -w 000.000.000.13[0-6] sudo collectl -scdn -p "'/var/log/collectl/*$COLLECTL_DTDEBUT*.raw.gz'" --from $COLLECTL_DTDEBUT:$COLLECTL_HRDEBUT-$COLLECTL_DTDEBUT:23:59 -oTm -f /var/log/collectl/tmp/Collectl -P

   echo "* Récupération des journaux Collectl. *"
   rpdcp -w 000.000.000.13[0-6] '/var/log/collectl/tmp/*.tab.gz' $REP_EXPORT/_collectl/

   # Journaux des noeuds.
   echo "* Récupération des journaux des noeuds. *"
   rpdcp -w 000.000.000.13[0-6] "$OBH/logs/ozone-USERNAME-datanode-servozone0*.log" $REP_EXPORT/_logs/
